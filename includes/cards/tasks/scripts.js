(function($) {
  window.dt_dashboard.onAdd('DT_Dashboard_Plugin_Tasks', function (context) {
    ...
    function build_tasks() {
      ...
      if (!html ){
         $(context.element).find('.existing-tasks').html(`<li>${window.lodash.escape(wpApiDashboard.translations.no_tasks)}</li>`)
      } else {
         $(context.element).find('.existing-tasks').html(html)
      }

       $(context.element).find('.complete-task').on("click", function () {
       ...
       $(context.element).find('.remove-task').on("click", function () {
         $(context.element).find('#tasks-spinner').addClass('active')
